...
class StudentController extends Controller
{
    ...
    public function show()
    {
        ...
        $sql = Students::with('user', 'class_section', 'category', 'father', 'mother', 'guardian', 'fees_paid')
    ...
    public function onlineRegistrationList()
    {
        ...
        $sql = Students::with('user', 'class', 'category', 'father', 'mother', 'guardian', 'fees_paid')->where('application_type', 'online')->ofTeacher()
            // Search query
            ->when($search, function ($query) use ($search) {
                $query->where(function ($query) use ($search) {
                    $query->where('user_id', 'LIKE', "%$search%")
        ...
        try {
            ...
            foreach ($examDetails as $examResult) {
                ...
                foreach ($subjectMarks as $mark) {
                    ...
                    if ($timetable?->date) {
                        try {
                            $examDateCarbons[] = Carbon::parse($timetable->date);
                        } catch (Throwable $e) {
